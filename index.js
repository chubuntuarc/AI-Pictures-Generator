...
async function getPromptsFromSheet() {
    ...
    try {
        // Initialize auth using GoogleAuth - this will automatically find
        // Application Default Credentials (ADC) or service account environment variables.
        ...
        try {
            ...
            if (credentials && credentials.client_email) {
                ...
                console.log("Authenticated, but could not determine specific user email (might be using ADC without a specific service account key).");
            }
        } catch (credError) {
            console.warn(`Could not retrieve credential details: ${credError.message}`);
        }
        // --- END NEW ---


        // Initialize the sheet - doc ID is the long id in the sheet URL
        const doc = new GoogleSpreadsheet(GOOGLE_SHEET_ID, auth); // Pass GoogleAuth instance
        ...
        await sheet.loadCells({ // Specify range using A1 notation
        ...
        const prompts = [];
        for (let i = 1; i < sheet.rowCount; i++) { // Start from row 1 (after header)
        ...
        if (filteredPrompts.length === 0) {
            return { success: false, error: `No prompts found in column ${PROMPT_COLUMN_INDEX + 1} of sheet '${SHEET_NAME}'.` };
        }
        ...
        let userErrorMessage = "An unexpected error occurred while accessing Google Sheets.";
        ...
        if (error.response && error.response.data && error.response.data.error) {
            ...
            detailedLog = `Google API Error: Status ${googleError.code} - ${googleError.message}. Details: ${JSON.stringify(googleError.details)}`;
            console.error(`Google API Error Details: ${JSON.stringify(googleError)}`);

            if (googleError.code === 403) {
                if (googleError.message.includes('permission denied') || googleError.status === 'PERMISSION_DENIED') {
                    userErrorMessage = "Permission denied accessing Google Sheet. Ensure the service account or your ADC user has access to the sheet.";
                    console.error("Specific Error: Permission Denied. Check sheet sharing settings.");
                } else if (googleError.message.includes('insufficient authentication scopes')) {
                     userErrorMessage = "Permission denied due to insufficient API scopes. Check the scopes requested in the code.";
                     console.error("Specific Error: Insufficient Scopes. Verify 'scopes' array in GoogleAuth.");
                } else if (googleError.message.toLowerCase().includes('api is not enabled') || googleError.status === 'SERVICE_DISABLED') {
                    userErrorMessage = "The Google Sheets API is not enabled for your project. Please enable it in the Google Cloud Console.";
                    console.error("Specific Error: Google Sheets API is likely not enabled for this project.");
                } else {
                    userErrorMessage = `Permission denied accessing Google Sheet (Code 403). Reason: ${googleError.message}`;
                    console.error(`Specific Error: General 403 Forbidden - ${googleError.message}`);
                }
            } else if (googleError.code === 404) {
                 userErrorMessage = "Google Sheet not found. Verify the GOOGLE_SHEET_ID is correct.";
                 console.error("Specific Error: Google Sheet Not Found (404). Check GOOGLE_SHEET_ID.");
            } else {
             ...
             detailedLog = `HTTP Error: Status ${error.response.status}. Data: ${JSON.stringify(error.response.data)}`;
             console.error(detailedLog);
             if (error.response.status === 403) {
                 userErrorMessage = "Permission denied accessing Google Sheet (HTTP 403). Check credentials and sheet permissions.";
                 console.error("Specific Error: HTTP 403 Forbidden. Could be credentials or permissions.");
             } else if (error.response.status === 404) {
                 userErrorMessage = "Google Sheet endpoint not found (HTTP 404). Check GOOGLE_SHEET_ID or API endpoint validity.";
                 console.error("Specific Error: HTTP 404 Not Found. Check GOOGLE_SHEET_ID.");
             } else {
                 userErrorMessage = `Server responded with error code ${error.response.status}.`;
             }
        } else if (error.message.includes('Unable to load service account credentials') || error.message.includes('Could not load the default credentials')) {
             userErrorMessage = "Could not find Application Default Credentials or Service Account Key. Ensure credentials are configured correctly.";
             console.error("Specific Error: Credential loading failed. Run 'gcloud auth application-default login' or check service account key path/environment variables.");
             detailedLog = error.message; // Keep the original message for this specific case
        } else if (error.name === 'TypeError' && error.message.includes('sheetsByTitle')) {
             userErrorMessage = "Failed to load sheet details, possibly due to an earlier authentication/permission issue. Check previous logs.";
             console.error("Specific Error: TypeError accessing sheetsByTitle. Likely caused by failed loadInfo() due to auth/permissions.");
...
async function generateImage(prompt, apiKey) {
    ...
    try {
        ...
        if (response.data && response.data.data && response.data.data.length > 0 && response.data.data[0].b64_json) {
            ...
            return { success: false, error: error_message, details: JSON.stringify(response.data) };
        }
...
if (!startupOk) {
    ...
    // process.exit(1); // Exit if critical variables are missing - uncomment if needed
     ...
     console.log("Attempting to use Application Default Credentials or Service Account for Google Sheets.");
}
